...
export default function ExpenseCard({ expense, userName, userColor, onDelete }: Props) {
  ...
  return (
    <>
      ...
      {/* Modal edición */}
      {showEdit && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          onClick={e => { if (e.target === e.currentTarget) setShowEdit(false); }}>
          <div style={{ backgroundColor: '#f0f4f8', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 680, maxHeight: '92vh', overflowY: 'auto', padding: '24px 20px 40px' }}>

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ fontSize: 20, fontWeight: 800, color: '#1e293b', margin: 0 }}>✏️ Editar gasto</h3>
              <button onClick={() => setShowEdit(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#64748b' }}>✕</button>
            </div>

            {expense.imageUri && (
              <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => { setShowEdit(false); setShowImage(true); }}>
                <img src={expense.imageUri} alt="ticket" style={{ width: '100%', maxHeight: 140, objectFit: 'cover', borderRadius: 12, opacity: 0.9 }} />
                <div style={{ textAlign: 'center', fontSize: 12, color: '#94a3b8', marginTop: 4 }}>Toca para ver el ticket completo</div>
              </div>
            )}

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>

              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>📅 Fecha</label>
                <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '2px solid #e2e8f0', fontSize: 15, outline: 'none', color: '#1e293b' }} />
              </div>

              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>💰 Importe</label>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input type="number" inputMode="decimal" value={editAmount} onChange={e => setEditAmount(e.target.value)}
                    style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: `2px solid ${BLUE}`, fontSize: 26, fontWeight: 800, color: '#1e293b', outline: 'none', textAlign: 'right', backgroundColor: BLUE_LIGHT }} />
                  <span style={{ fontSize: 24, fontWeight: 800, color: BLUE }}>€</span>
                </div>
              </div>

              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>📝 Descripción</label>
                <input type="text" value={editDesc} onChange={e => setEditDesc(e.target.value)}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '2px solid #e2e8f0', fontSize: 15, outline: 'none' }} />
              </div>

              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>📂 Categoría</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {CATEGORIES.map(cat => (
                    <button key={cat.value} onClick={() => setEditCat(cat.value)} style={{
                      ...
                      border: `2px solid ${editCat === cat.value ? BLUE : '#e2e8f0'}`,
                      backgroundColor: editCat === cat.value ? BLUE_LIGHT : '#f8fafc',
                    }}>
                      <span style={{ fontSize: 22 }}>{cat.icon}</span>
                      <span style={{ fontSize: 10, color: editCat === cat.value ? BLUE : '#64748b', fontWeight: editCat === cat.value ? 700 : 400 }}>{cat.label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>👤 Quién paga</label>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {users.map(user => (
                    <button key={user.id} onClick={() => setEditUser(user.id)} style={{
                      ...
                      border: `2px solid ${editUser === user.id ? user.color : '#e2e8f0'}`,
                      backgroundColor: editUser === user.id ? user.color : '#f8fafc',
                      color: editUser === user.id ? '#fff' : '#475569',
                    }}>
                      {user.name}
                    </button>
                  ))}
              ...
              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 18, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>🛍️ Productos ({editItems.length})</label>
                {editItems.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: i < editItems.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    <span style={{ flex: 1, fontSize: 13, color: '#1e293b' }}>{item.name}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: BLUE, minWidth: 55, textAlign: 'right' }}>{item.totalPrice.toFixed(2)} €</span>
                    <button onClick={() => setEditItems(prev => prev.filter((_, j) => j !== i))}
                      style={{ background: 'none', border: 'none', fontSize: 16, cursor: 'pointer', color: '#cbd5e1' }}>✕</button>
                  </div>
                ))}
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                  ...
                  <input type="number" placeholder="€" value={newPrice} onChange={e => setNewPrice(e.target.value)} onKeyDown={e => e.key === 'Enter' && addEditItem()}
                    style={{ width: 64, padding: '8px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, outline: 'none', textAlign: 'right' }} />
                  <button onClick={addEditItem} style={{ padding: '8px 12px', borderRadius: 8, border: 'none', backgroundColor: BLUE, color: '#fff', fontWeight: 700, fontSize: 18, cursor: 'pointer' }}>＋</button>
                </div>
              </div>

              <button onClick={saveEdit} style={{ width: '100%', padding: 18, borderRadius: 14, backgroundColor: BLUE, color: '#fff', fontWeight: 800, fontSize: 17, border: 'none', cursor: 'pointer', boxShadow: '0 4px 16px rgba(37,99,235,0.35)' }}>
                💾 Guardar cambios
              </button>
            </div>
          </div>
