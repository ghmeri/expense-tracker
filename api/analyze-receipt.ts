...
Reglas:
- Incluye solo los productos comprados con precio positivo
- Ignora descuentos (importes negativos), impuestos (IVA/IGF), subtotales, forma de pago, puntos
- Para productos por peso (ej: "0.424kg x 5.99/kg = 2.54") usa el precio final (2.54) y el nombre de la línea anterior
- Normaliza los nombres: primera letra mayúscula, resto minúsculas
- El ticket puede estar en español, catalán u otro idioma
- total = importe final pagado (línea TOTAL)`;
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!apiKey) {
    return new Response(
      JSON.stringify({ error: 'Falta la clave OPENAI_API_KEY. Añádela en Vercel → Settings → Environment Variables y vuelve a desplegar.' }),
      { status: 500, headers: CORS }
    );
  }
  ...
  try {
    ...
    if (!openaiRes.ok) {
      ...
      return new Response(JSON.stringify({ error: `Error OpenAI ${openaiRes.status}: ${txt}` }), { status: 500, headers: CORS });
    }
